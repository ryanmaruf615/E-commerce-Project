<?php
    ...
    if(isset($_POST) && !empty($_POST)){
      ...
      //display the errors or update database
      if(!empty($errors)){
//display errors
...
//update database
        ...
        if(isset($_GET['edit'])){
        $updatesql="UPDATE categories SET category ='$category', parent='$post_parent' WHERE id='$id' ";
        }
        $db->query($updatesql);
        header('Location:categories.php');
      }
...
<div class ="row">
  ...
  <form class="form" action="categories.php"<?=((isset($_GET['edit']))?'?edit='.$edit_id:'');?> method="post">
    ...
    <div class="form-group">
      ...
      <option value="0"<?=(($parent_value == 0)?' seleted="seleted"':'');?>>Parent</option>
      <?php while($parent=mysqli_fetch_assoc($result)): ?>
        <option value="<?=$parent['id'];?>"<?=(($parent_value == $parent['id'])?' seleted="seleted"':'');?>><?=$parent['category'];?></option>
      <?php endwhile;?>
    </select>
    </div>
